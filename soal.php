...
<?php if ($pg == 'soal') { ?>
    ...
    foreach ($decoded as $soal) {
        if ($soal['id_soal'] == $pengacak[$nosoal]) {
            $jawab = fetch($koneksi, 'jawaban_temp', array('id_siswa' => $id_siswa, 'id_mapel' => $id_mapel, 'id_soal' => $soal['id_soal'], 'id_ujian' => $ac));
    ?>
            <div class='box-body'>
                <div class='row'>
                    <div class='col-md-12'>
...
if ($pg == 'jawab') {
    ...
    $cekjawaban = rowcount($koneksi, 'jawaban_temp', $where);
    if ($jenis == 1) {
        if ($cekjawaban == 0) {
            $exec = insert($koneksi, 'jawaban_temp', $data);
        } else {
            $exec = update($koneksi, 'jawaban_temp', $data, $where);
        }
    } else {
        if ($cekjawaban == 0) {
            $exec = insert($koneksi, 'jawaban_temp', $dataesai);
        } else {
            $exec = update($koneksi, 'jawaban_temp', $dataesai, $where);
        }
    }
    echo $exec;
} elseif ($pg == 'ragu') {
    ...
    $cekragu = fetch($koneksi, 'jawaban_temp', $where);
    if ($cekragu['ragu'] == 0) {
        $exec = update($koneksi, 'jawaban_temp', array('ragu' => 1), $where);
    } else {
        $exec = update($koneksi, 'jawaban_temp', array('ragu' => 0), $where);
    }
    echo $exec;
}
?>