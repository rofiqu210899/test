...
<head>
	...
	<style>
		.fade-to-right {
			background: linear-gradient(to right, rgba(255, 255, 255, 0) 0%, rgba(255, 255, 255, 0.4) 60%, rgba(255, 255, 255, 1) 100%);
		}
...
<body class="min-h-screen bg-slate-100/70 font-sans text-slate-800 antialiased relative flex items-center justify-center p-3 sm:p-6 lg:p-10 selection:bg-emerald-500 selection:text-white">

	<!-- Outer Card (Clean White, Rounded-3xl, Elevated Shadow) -->
	<main class="relative z-10 w-full max-w-5xl my-auto bg-white rounded-[2rem] sm:rounded-[2.5rem] shadow-2xl shadow-slate-300/60 border border-slate-100 overflow-hidden grid grid-cols-1 lg:grid-cols-12 min-h-[560px]">
		
		<!-- Left Panel: Uploaded Background with Seamless Fade-to-White (5 cols on LG) -->
		<div class="relative lg:col-span-5 bg-slate-50 p-6 sm:p-10 flex flex-col justify-between overflow-hidden min-h-[260px] lg:min-h-[560px]">
			...
			<div class="relative z-10">
				<div class="w-11 h-11 rounded-2xl bg-white/90 p-1.5 shadow-sm ring-1 ring-slate-200/60 backdrop-blur-md flex items-center justify-center overflow-hidden">
			...
			<div class="relative z-10 mt-auto pt-6 text-left">
				<h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-slate-900 tracking-tight leading-[1.15]">
		...
		<!-- Right Panel: Centered Clean Login Form (7 cols on LG) -->
		<div class="lg:col-span-7 p-6 sm:p-10 lg:p-14 flex flex-col justify-between bg-white relative">
			...
			<div class="w-full max-w-sm mx-auto my-auto text-center py-4">
				...
				<!-- Center Logo Icon -->
				<div class="w-12 h-12 mx-auto mb-4 rounded-2xl bg-slate-100 p-2 flex items-center justify-center ring-1 ring-slate-200/80 shadow-xs">
					<img src="<?= $logo_src ?>" onerror="this.onerror=null; this.src='<?= $homeurl ?>/dist/img/tutwuri.jpg';" alt="Logo" style="max-width: 32px; max-height: 32px; width: 100%; height: 100%; object-fit: contain;">
				</div>
				...
				<p class="text-xs text-slate-400 mt-1 mb-7">
					Login to Your Account
				</p>

				<!-- Form Inputs -->
				<form id="formlogin" action="ceklogin.php" method="POST" class="space-y-3.5" autocomplete="off">
					...
					<div class="relative">
						<input type="text" id="username" name="username" required autocomplete="username" placeholder="eg. username / no. peserta"
							class="w-full px-5 py-3.5 rounded-full bg-slate-100/90 hover:bg-slate-100 text-slate-800 placeholder-slate-400 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white border border-transparent focus:border-emerald-500/30 transition-all">
					</div>
					...
					<div class="relative">
						<input type="password" id="password" name="password" required autocomplete="current-password" placeholder="password"
							class="w-full pl-5 pr-12 py-3.5 rounded-full bg-slate-100/90 hover:bg-slate-100 text-slate-800 placeholder-slate-400 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:bg-white border border-transparent focus:border-emerald-500/30 transition-all">
					...
					<div class="pt-2">
						<button type="submit" id="btnSubmit" class="w-full py-3.5 px-6 rounded-full bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98] text-white font-semibold text-sm shadow-md shadow-emerald-600/25 transition-all duration-200 flex items-center justify-center gap-2 cursor-pointer disabled:opacity-75 disabled:cursor-not-allowed">
							<span id="btnText">Sign In</span>
							<svg id="btnSpinner" class="w-4 h-4 animate-spin hidden text-white" fill="none" viewBox="0 0 24 24">
